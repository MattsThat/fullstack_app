...
const HostUserInfo = require("./HostUserInfo");
...
router.post("/updateHostUserInfo", (req, res) => {
  ...
  HostUserInfo.findOneAndUpdate({'id':id}, update, err => {
    if (err){
...
router.delete("/deleteHostUserInfo", (req, res) => {
  const { id } = req.body;
  HostUserInfo.findOneAndDelete({'id':id}, err => {
...
router.get("/selectHostUserInfo", (req, res) => {
    console.log('id selectHostUserInfo',req.query.id);
    const id  = req.query.id;
    HostUserInfo.findOne({'id' : id}, function(err,data){
        if(err || data === null){
...
router.post("/putHostUserInfo", (req, res) => {
  let data = new HostUserInfo();
  ...
  try {
    ...
    req.app.get('db').collection('HostUserInfo').insertOne(resdata)
